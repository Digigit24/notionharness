...
/** Resolves one doc's mapped properties — used by the rows-as-pages fetch (a
 * paired page's header shows this record's current field values). */
export async function GET(_req: Request, { params }: { params: Promise<{ collection: string; id: string }> }) {
  ...
  const doc = await payload
    .findByID({ collection: collection as 'pages', id: recordId, depth: 0, overrideAccess: true, disableErrors: true })
    .catch(() => null)
  if (!doc) {
