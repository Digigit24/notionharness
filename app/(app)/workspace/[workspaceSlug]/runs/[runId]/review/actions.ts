...
export async function addReviewCommentAction(input: {
  ...
  if (body.length > 10_000) throw new Error('A comment must be under 10,000 characters.')
...
export async function sendReviewCommentsAction(
  ...
  note: string,
): Promise<SendReviewCommentsResult> {
  const user = await getCurrentPayloadUser()
  if (!user) throw new Error('You must be logged in to send review comments.')
  ...
  if (comments.length === 0 && !note.trim()) {
    throw new Error('Nothing to send — add a line comment or a note first.')
  }
  ...
  if (run.taskId == null || run.agentId == null) {
    throw new Error('This run has neither a conversation nor a task/agent to send the review to.')
  }
