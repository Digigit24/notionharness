...
/**
 * ROADMAP B-4 "Work" — hand-added, NOT generated by `payload generate:types`
 * (see collections/SavedViews.ts's header comment for why). Mirrors what
 * `payload generate:types` would produce for that collection's fields; a
 * human must re-run the real generator once the migration is applied, which
 * will supersede this hand-written block.
 *
 * This interface was referenced by `Config`'s JSON-Schema
...
/**
 * ROADMAP B5.3 (Batch B-5 "Attention") — hand-added, NOT generated by
 * `payload generate:types` (see collections/PushSubscriptions.ts's header
 * comment for why). Mirrors what `payload generate:types` would produce for
 * that collection's fields; a human must re-run the real generator once the
 * migration is applied, which will supersede this hand-written block.
 *
 * This interface was referenced by `Config`'s JSON-Schema
...
/**
 * ROADMAP B5.3 (Batch B-5 "Attention") — hand-added alongside
 * `PushSubscription` above; see that interface's comment for why this isn't
...
/**
 * ROADMAP B-4 "Work" — hand-added alongside `SavedView` above; see that
 * interface's comment for why this isn't `payload generate:types` output.
 *
 * This interface was referenced by `Config`'s JSON-Schema
...
/**
 * ROADMAP B5.3 (Batch B-5 "Attention") — hand-added alongside
 * `NotificationPreference` above; see that interface's comment for why this
 * isn't `payload generate:types` output.
 *
 * This interface was referenced by `Config`'s JSON-Schema
