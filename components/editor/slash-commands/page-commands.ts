...
import { getAskAgentPageHandler } from '../agent-thread/registry'
  ...
  if (!alreadyPresent(widget, 'Ask agent')) {
    widget.config.items.push({
      ...
      action: (ctx) => {
        const { rootComponent, model } = ctx
        const { host, doc } = rootComponent
        const handler = getAskAgentPageHandler()
        if (!handler) {
          // ROADMAP B3.5 — the docked page-agent panel is the parallel,
          // not-yet-merged `b3-docked-agent` branch's deliverable; see
          // `agent-thread/registry.ts`'s `registerAskAgentPageHandler` doc
          // comment for the full decoupling seam. Warn, don't silently no-op.
          console.warn('[ask-agent-page] No handler registered — the docked page-agent panel (b3-docked-agent) has not mounted.')
          return
        }
        const anchorElement = host.view.getBlock(model.id) ?? host
        void handler({ doc, host, pageId: pageIdFromDoc(doc), anchorElement })
      },
